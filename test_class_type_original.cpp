#include "builtin.hpp"
#include "test_class_type.hpp"

namespace __test_class_type__ {
...
__ss_int test_local_student() {
    ...
    FAST_FOR(i,0,__ss_int(100000),1,0,1)
        s = (new Student(const_0, __ss_int(20)));
        total = (total+s->age);
    END_FOR
...
void __init() {
    ...
    if (__eq(__test_class_type__::__name__, const_1)) {
        result = test_local_student();
        print(__test_class_type__::result);
    }
}
...
int main(int, char **) {
    __shedskin__::__init();
    __shedskin__::__start(__test_class_type__::__init);
}
